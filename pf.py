    ...
    for i in range(len(df)):
        ...
        if month == 1:
            if year < 2030:
                ...
                df.loc[df.index[i], 'Basic'] = df.loc[df.index[i], 'Basic'] * 1.86 * (1.03 ** 3)
            elif 2031 <= year <= 2039:
                ...
                # Apply pay commission multiplication factor and 3 years of increments
                df.loc[df.index[i], 'Basic'] = df.loc[df.index[i], 'Basic'] * 1.4 * (1.03 ** 3)
            elif year >= 2041:
...
def display_monthly_ledger(projection_df):
    ...
    if not filtered_df.empty:
        ...
        data=df_to_excel_bytes(filtered_df[display_cols]),  # You'll need this helper function
